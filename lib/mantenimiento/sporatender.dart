// Tabla de Servicios por atender
...
class SPorAtender extends StatelessWidget {
  const SPorAtender({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: Scaffold(
        appBar: AppBar(
          title: const Text('Servicios por atender'),
        ),
        drawer: const MenuPage(),
        body: const TSPorAtender(),
      ),
    );
  }
}

class TSPorAtender extends StatelessWidget {
  const TSPorAtender({Key? key}) : super(key: key);
              ...
              children: [
                const Center(
                  child: Column(
                    ...
                    children: [
                      Text(
                        'No tenés servicios por atender.',
                        style: TextStyle(
                  ...
                  child: FloatingActionButton(
                    onPressed: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => const SPorAtender(),
                        ),
                      );
                    },
  ...
  Future<Map<String, dynamic>> executeQuery() async {
    ...
    try {
      final result = await connection.query(
          'SELECT id, "Servicio_Codigo", "Descripcion", "Maquina_id", "FechaInicio", "Iniciado", "Pendiente" FROM servicio WHERE id IN (SELECT "idServicio" FROM "ServicioXUsuario" WHERE "idUser" = $idUser) AND "Realizado" = False AND "Eliminado" IS NULL ORDER BY "Creado" DESC');
